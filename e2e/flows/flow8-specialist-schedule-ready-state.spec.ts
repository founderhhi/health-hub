...
test.describe('Flow 8: specialist schedule editor and ready consultation lifecycle', () => {
  ...
  test('keeps specialist scheduling and patient join state in sync', async ({ browser, request }) => {
    ...
    await expect
      .poll(async () => {
        const latestReferral = await getReferral(request, specialistAuth, referral.id);
        return latestReferral.consultation_mode;
      })
      .toBe('offline');
    ...
    await expect(specialistPage.getByText(location)).toBeVisible();
    ...
    await expect
      .poll(async () => {
        const latestReferral = await getReferral(request, specialistAuth, referral.id);
        return latestReferral.consultation_status || '';
      })
      .toBe('ready');
